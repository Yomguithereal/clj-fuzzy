...
;; Lookup functions
(defn- lookup-vowel [pos] (if (= 0 pos) [:A :A 1] [nil nil 1]))
...
(defn- lookup-C-3 [string pos]
  (cond (and (> pos 0)
        ...
        (or (in? (slice string 0 4) ["VAN " "VON "])
            ...
            (and (or (= pos 0)
                     (in? (slice string (- pos 1) 1) ["A" "O" "U" "E"]))
...
(defn- lookup-C-6 [string pos]
  (if (and (re-test? #"^I|E|H$" (slice string (+ pos 2) 1))
    ...
    (if (or (and (= pos 1)
                 (= "A" (slice string (- pos 1) 1)))
            (re-test? #"^UCCE(E|S)$" (slice string (- pos 1) 5)))
      [:KS :KS 3]
      [:X :X 3])
    [:K :K 2]))
...
(defn lookup-G-1-bis [string pos]
  (cond (and (> pos 2)
        ...
        (and (> pos 0) (not= "I" (slice string (- pos 1) 1))) [:K :K 2]
        :else [nil nil 2]))

(defn- lookup-G-1 [string pos]
  (cond (and (> pos 0)
             (not (vowel? (slice string (- pos 1) 1)))) [:K :K 2]
  ...
  (let [current (if (= "J" (slice string (+ pos 1) 1)) 2 1)]
    (if (and (= pos 0) (not= "JOSE" (slice string pos 4)))
      ...
      (if (and (vowel? (slice string (- pos 1) 1))
               (not (slavo-germanic? string))
               (re-test? #"^A|O$" (slice string (+ pos 1) 1)))
...
(defn- lookup-S [string pos lastp]
  (cond (re-test? #"^(I|Y)SL$" (slice string (- pos 1) 3)) (lookup-S-1)
        (and (= pos 0)
